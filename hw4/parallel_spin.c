...
mutex:  12.8    7.12    6.6     6.5     6.8     7.4
spin:   12.7    9.17    6.4     6.9     6.43    6.3

In the time data above, we could find out mutex will take more time to retrieve the data by giving it more threads, while the spin_lock will not.
In my opinion, it's because of the mechanism behind the mutex.

Mutex will block the thread, and put it into sleep when it finds a conflict. 
Spin-lock, on the other hand, will not block the thread. It will keep spining for the resource.

Therefore, if we need to keep waking up the thread, it will result in increasing the time.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
